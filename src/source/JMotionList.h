...
public:
  ...
  void PreMotion(){
    for(unsigned c=0;c<Nref;c++)MotionData[c].PreMotion();
  }

  //-Only for simple motion.
  void Sp_Movedt(unsigned ref,const tdouble3&  mvsimple,double dt){
    MotionData[ref].Sp_Movedt(mvsimple,dt);
  }
  void Sp_Movedt(unsigned ref,const tmatrix4d& mvmatrix,double dt){
    MotionData[ref].Sp_Movedt(mvmatrix,dt);
  }
  ...
  void Ace2_PosMotion(double dt){
    for(unsigned c=0;c<Nref;c++)MotionData[c].Ace2_PosMotion(dt);
  }

  bool GetData(unsigned ref,bool& typesimple,tdouble3& simplemov,tdouble3& simplevel
    ,tdouble3& simpleace,tmatrix4d& matmov,tmatrix4d& matmov2)const;
