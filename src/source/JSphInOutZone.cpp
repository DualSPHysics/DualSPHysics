...
/// Constructor.
//==============================================================================
JSphInOutAwas::JSphInOutAwas(JLog2 *log,unsigned idzone,double inletx,tdouble3 inletdir
...
//==============================================================================
void JSphInOutAwas::ReadXml(const JXml *sxml,TiXmlElement* ele
  ,const std::string &dirdatafile,JGaugeSystem *gaugesystem)
{
  ...
  if(GaugeX==DBL_MAX){
    ...
    if(GaugeXh!=DBL_MAX)GaugeX=GaugeXh*gaugesystem->GetH();
...
//==============================================================================
void JSphInOutAwas::GetConfig(std::vector<std::string> &lines)const{
...
//==============================================================================
float JSphInOutAwas::GetVelCorr(double timestep){
...
//==============================================================================
void JSphInOutAwas::SaveCsvData(){
  ...
  ,const JSphPartsInit *partsdata,JGaugeSystem *gaugesystem)
{
  ...
  if(VelMode==MVEL_Interpolated){
    TiXmlElement* xele=ele->FirstChildElement("imposevelocity");
    if(xele)xele=xele->FirstChildElement("awas");
    if(sxml->CheckElementActive(xele)){
      AwasVel=new JSphInOutAwas(Log,IdZone,PtPlane.x,Direction,GravityZ,dirdatafile
        ,gaugesystem,sxml,xele);
    }
  }
  //<vs_inawwas_end>
