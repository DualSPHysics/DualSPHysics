...
/// Constructor.
//==============================================================================
JGaugeFlow::JGaugeFlow(unsigned idx,std::string name,const jmsh::StMeshBasic& meshbas
  ,float kclimit,unsigned buffersize
  ,int gpucount)
  :JGaugeItem(GAUGE_Flow,idx,name,gpucount,(buffersize<1? 30: buffersize))
{
  ClassName="JGaugeFlow";
  FileInfo=string("Saves flow data measured from fluid particles (by ")+ClassName+").";
  MeshDat=NULL;
  DpArea=0.0;
  KcLimit=kclimit;
  Reset();
...
/// Reset GPU memory.
//==============================================================================
void JGaugeFlow::ResetGpuMemory(){
  for(int g=0;g<GpuCount;g++){
    if(AuxDataGpu[g].GpuMemory)
      Run_Exceptioon(fun::PrintStr("Auxiliary GPU memory for unit %d is not free.",g));
    AuxDataGpu[g]=StrGaugeFlowDataGpu();
  }
}
#endif
...
/// Configures mesh positions for calculation.
//==============================================================================
void JGaugeFlow::SetMeshData(const jmsh::StMeshBasic& meshbas){
  //-Free memory.
  delete   MeshDat; MeshDat=NULL;
  //delete[] MassDatCpu; MassDatCpu=NULL;
  //-Configures mesh definition.
...
/// Returns structure with basic information about configuration.
//==============================================================================
JGaugeFlow::StInfo JGaugeFlow::GetInfo()const{
  StInfo v={TDouble3(0),TDouble3(0),TDouble3(0),TDouble3(0),TDouble2(0),TUint3(0),TFloat3(0)};
  const jmsh::StMeshBasic& mb=MeshBas;
  const jmsh::StMeshPts& m=MeshPts;
  v.ptref=m.ptref;
  v.ptend=m.ptref+(m.vdp1*m.npt1)+(m.vdp2*m.npt2);
  v.vec1=mb.vec1;
  v.vec2=mb.vec2;
  v.dispt=TDouble2(mb.dispt1,mb.dispt2);
  v.npt=TUint3(m.npt1,m.npt2,m.npt);
  v.dirdat=m.dirdat;
  return(v);
}
...
/// Configuration of initial limits of calculation area.
//==============================================================================
void JGaugeFlow::ConfigDomMCel(bool fixed){
  if(!fixed)Run_Exceptioon("Unfixed calculation domain is invalid for Mesh-Gauge.");
  FixedDomMCel=fixed;
  const jmsh::StMeshPts& m=MeshPts;
  const tdouble3 ptref=m.ptref;
  const tdouble3 ptend=m.ptref+(m.vdp1*m.npt1)+(m.vdp2*m.npt2);
  const tdouble3 psmin=MinValues(ptref,ptend);
  const tdouble3 psmax=MaxValues(ptref,ptend);
  CalcMCelIniFinFromPos(psmin,psmax,DomMCelIni0,DomMCelFin0);
}
...
/// Record the last measure result.
//==============================================================================
void JGaugeFlow::StoreResult(){
  if(OutputSave){
    //-Allocates memory for OutSize records.
    while(unsigned(OutBuff.size())<OutSize){
      OutBuff.push_back(StrGaugeFlowRes());
    }
    //-Empty buffer.
    if(OutCount+1>=OutSize)SaveResults();
    //-Stores last results.
  ...
  if(OutCount){
	  ...
	  if(first){
		...
		if(KcLimit == FLT_MAX){
			...
			kclimit_str = to_string(KcLimit);
		}
		scsv << "KcLimit: " << kclimit_str << jcsv::Endl();
		scsv << "time [s];flow [l/s] " << jcsv::Endl();
	  }
	  scsv.SetData();
	  scsv << jcsv::Fmt(jcsv::TpFloat1,"%g") << jcsv::Fmt(jcsv::TpFloat1,"%g");
	  for(unsigned c=0;c<OutCount;c++){
		  scsv << OutBuff[c].timestep << OutBuff[c].flow << jcsv::Endl();
	  }
      OutCount=0;
  }
}
...
/// Saves last result in VTK file.
//==============================================================================
void JGaugeFlow::SaveVtkResult(unsigned cpart){
  if(JVtkLib::Available()){
    //-Saves VTK with npt size data.
    Log->AddFileInfo(fun::FileNameSec(GetResultsFileVtk(),UINT_MAX),FileInfo);
    jmsh::JMeshTDatasSave::SaveVtk(GetResultsFileVtk(),int(cpart),MeshDat,false);
...
/// Loads and returns number definition points.
//==============================================================================
unsigned JGaugeFlow::GetPointDef(std::vector<tfloat3>& points)const{
  //printf("=> GetPointDef  MeshPts.npt:%u \n",MeshPts.npt);
  const unsigned np=MeshPts.npt;
  tfloat3* pos=new tfloat3[np];
  MeshDat->GetPosf(np,pos);
...
/// Creates VTK file with the scheme of gauge configuration.
//==============================================================================
void JGaugeFlow::SaveVtkScheme()const{
  const string filevtk=AppInfo.GetDirOut()+"CfgGaugeFlow_"+Name+".vtk";
...
/// Calculates velocity at indicated points (on CPU).
//==============================================================================
template<TpKernel tker> void JGaugeFlow::CalculeCpuT(const StDataCpu& datacpu){
  //-Prepare input data.
  double flowvalue=0;
  const double& timestep =datacpu.timestep;
  const StDivDataCpu& dvd=datacpu.dvd;
  ...
  //-Start measure.
  //MeshDat->ClearData();  //-It is not necessary.
  float* datavdir =MeshDat->GetVarFloat("VelDir");// DELETE
  //-Obtains basic data for calculations.
  const jmsh::StMeshPts& m=MeshPts;
  const unsigned npt1=m.npt1;
  const unsigned npt2=m.npt2;
  ...
  for(int cp=0;cp<npt;cp++){
    ...
    //-Stores results.
	flowvalue+=(sumvel.x*vdir.x + sumvel.y*vdir.y + sumvel.z*vdir.z);
  }
...
/// Calculates data at indicated mesh points (on CPU).
//==============================================================================
void JGaugeFlow::CalculeCpu(const StDataCpu& datacpu){
  switch(CSP.tkernel){
    case KERNEL_Cubic:       //Kernel Wendland is used since Cubic is not available.
    case KERNEL_Wendland:    CalculeCpuT<KERNEL_Wendland>  (datacpu);  break;
    default: Run_Exceptioon("Kernel unknown.");
...
/// Check if GPU memory was allocated.
//==============================================================================
bool JGaugeFlow::AllocatedGpuMemory(int id)const{
  if(id>=GpuCount)Run_Exceptioon("Id is invalid.");
  return(AuxDataGpu[id].GpuMemory);
}
...
/// Frees GPU memory.
//==============================================================================
void JGaugeFlow::FreeGpuMemory(int id){
  if(id>=GpuCount)Run_Exceptioon("Id is invalid.");
  StGaugeFlowDataGpu& aug=AuxDataGpu[id];
  aug.GpuMemory=false;
  //if(aug.Resultg   )cudaFree(aug.Resultg   ); aug.Resultg=NULL;
  if(aug.DataVdirg )cudaFree(aug.DataVdirg ); aug.DataVdirg=NULL;
...
/// Allocates GPU memory.
//==============================================================================
void JGaugeFlow::AllocGpuMemory(int id){
  if(id>=GpuCount)Run_Exceptioon("Id is invalid.");
  StGaugeFlowDataGpu& aug=AuxDataGpu[id];
  if(aug.GpuMemory)FreeGpuMemory(id);
  //fcuda::Malloc(&aug.Resultg,1);
  const unsigned npt12=MeshPts.npt1*MeshPts.npt2;
  const unsigned npt  =MeshPts.npt;
  fcuda::Malloc(&aug.DataVdirg ,npt);
  const unsigned saux=curedus::GetAuxSize_ReduSumFloat(npt);
...
/// Calculates data at indicated mesh points (on GPU).
//==============================================================================
void JGaugeFlow::CalculeGpu(const StDataGpu& datagpu){
  const int id=0;
  const int indomain=2;
  //-Run GPU calculation.
  if(indomain==2){
    ...
    StGaugeFlowDataGpu& aug=AuxDataGpu[id];
    if(!aug.GpuMemory)AllocGpuMemory(id);
    cugauge::ComputeGaugeFlow(CSP,dvd,MeshPts,KcLimit,posxy,posz,code
      ,velrho,aug.DataVdirg);
    Check_CudaErroor("Failed in GaugeFlow calculation.");
    //-Copy data from GPU memory.
    const unsigned npt=MeshPts.npt;
    //if(aug.DataVdirg )cudaMemcpy(datavdir ,aug.DataVdirg ,sizeof(float )*npt  ,cudaMemcpyDeviceToHost);
    //Check_CudaErroor("Failed in GaugeFlow memcpy.");
    //Computing flow
    //}
    float velsum=0;
