...
//------------------------------------------------------------------------------
/// Calculates the infinity norm of a 4x4 matrix  // SHABA new function
//------------------------------------------------------------------------------
__device__ float InfNorm4x4(tmatrix4f mat)
{
    float infnorm = 0.f;
...
//------------------------------------------------------------------------------
/// Calculates the infinity norm of a 3x3 matrix  // SHABA new function
//------------------------------------------------------------------------------
__device__ float InfNorm3x3(tmatrix3f mat)
{
    float infnorm = 0.f;
...
//------------------------------------------------------------------------------
/// Perform Pressure cloning for mDBC  // SHABA new function
//------------------------------------------------------------------------------
__device__ float KerPressClone(bool sim2d, const float rhog, float3 bnormal, const float3 grav, const float3 motace, const float3 dpos)
{
...
//------------------------------------------------------------------------------
/// Perform interaction between ghost node of selected bondary and fluid. // SHABA new function
//------------------------------------------------------------------------------
template<TpKernel tker,bool sim2d,TpSlipMode tslip>
  __global__ void KerInteractionMdbcCorrection_Fastv2
  ...
  ,const float3* boundnor,const float3* motionvel,const float3* motionace
  ,float4* velrho,float* boundonoff, float3 gravity)
{
  const unsigned p1=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p1<n){
    float3 bnormalp1=boundnor[p1];
    ...
    if (p1 < n) {
        ...
        if (bnormalp1.x != 0 || bnormalp1.y != 0 || bnormalp1.z != 0) {
            ...
            if (sumwab >= mdbcthreshold) {
                ...
                if (tslip == SLIP_Vel0) {//-DBC vel=0
                    velrho[p1].w = rhopfinal;
                }
                if (tslip == SLIP_NoSlip) {//-No-Slip
                    const float3 v = motionvel[p1];
                    velrho[p1] = make_float4(v.x + v.x - velrhofinal.x, v.y + v.y - velrhofinal.y, v.z + v.z - velrhofinal.z, rhopfinal);
                }
                if (tslip == SLIP_FreeSlip) {//-No-Penetration and free slip    SHABA
                    float3 FSVelFinal; // final free slip boundary velocity
                    const float3 v = motionvel[p1];
                    ...
                    // Save the velocity and density
                    velrho[p1] = make_float4(FSVelFinal.x, FSVelFinal.y, FSVelFinal.z, rhopfinal);
                }
            }
        }
    }
...
//==============================================================================
/// Calculates extrapolated data on boundary particles from fluid domain for mDBC. // SHABA new function for no-slip
/// Calcula datos extrapolados en el contorno para mDBC.
//==============================================================================
template<TpKernel tker, bool sim2d, TpSlipMode tslip> void Interaction_MdbcCorrectionT3
(bool fastsingle, unsigned n, unsigned nbound, float mdbcthreshold
    ...
    , const float3* motionace, float4* velrho, float* boundonoff, const tfloat3 gravity
    , cudaStream_t stm)
{
    const int2* beginendcellfluid = dvd.beginendcell + dvd.cellfluid;
    ...
    if (n) {
        ...
        KerInteractionMdbcCorrection_Fastv2 <tker,sim2d,tslip> << <sgridb,bsbound,0,stm >> >
            ...
            ,velrho,boundonoff,Float3(gravity));
    }
}
//==============================================================================
  ...
  ,cudaStream_t stm)
{
    ...
    case SLIP_NoSlip:{ const TpSlipMode tslip=SLIP_NoSlip; // SHABA chnaged to t3
      if(simulate2d)Interaction_MdbcCorrectionT3 <tker,true ,tslip> (fastsingle,n,nbound,mdbcthreshold,dvd,mapposmin,posxy,posz,poscell,code,idp,boundnor,motionvel,motionace,velrho,boundonoff,gravity,stm);
      else          Interaction_MdbcCorrectionT3 <tker,false,tslip> (fastsingle,n,nbound,mdbcthreshold,dvd,mapposmin,posxy,posz,poscell,code,idp,boundnor,motionvel,motionace,velrho,boundonoff,gravity,stm);
    }break;
...
//------------------------------------------------------------------------------
template<bool periactive> __global__ void KerCopyMotionVelAce(unsigned n
  ,double dt,const unsigned* ridpmot,const float4* velrho
  ,float3* motionvel,float3* motionace) //SHABA
{
  unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    int pid=ridpmot[p];
  ...
  ,const float4* velrho,float3* motionvel,float3* motionace)
{
  ...
  KerCopyMotionVelAce<true> <<<sgrid,SPHBSIZE>>> (nmoving,dt,ridpmot,velrho
    ,motionvel,motionace);
}
...
__global__ void KerPeriodicDuplicateNormals(unsigned n,unsigned pini
  ,const unsigned* listp,float3* normals,float3* motionvel,float3* motionace) //SHABA
{
  const unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    const unsigned pnew=p+pini;
