  ...
  ,const float4* velrhop,float4* shiftposfs)
{
  ...
  if(p<n){
    ...
    if(rs.x!=FLT_MAX){
      ...
      rs.x=(fabs(shiftdistx)<maxdist? shiftdistx: (shiftdistx>=0? maxdist: -maxdist))/dt;
      rs.y=(fabs(shiftdisty)<maxdist? shiftdisty: (shiftdisty>=0? maxdist: -maxdist))/dt;
      rs.z=(fabs(shiftdistz)<maxdist? shiftdistz: (shiftdistz>=0? maxdist: -maxdist))/dt;
    }
