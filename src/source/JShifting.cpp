...
/// \file JShifting.cpp \brief Implements the class \ref JShifting.
...
/// Constructor.
//==============================================================================
JShiftingZone::JShiftingZone(unsigned id,const tdouble3 &posref,const tdouble3 &vx
...
//==============================================================================
void JShiftingZone::PrepareZone(){
...
/// Constructor.
//==============================================================================
JShifting::JShifting(bool simulate2d,double dp,float h,JLog2* log)
...
//==============================================================================
std::string JShifting::GetShiftingModeStr()const{
  string tx;
...
//==============================================================================
void JShifting::ConfigBasic(TpShifting shiftmode,float shiftcoef,float shifttfs){
  ShiftMode=shiftmode;
  ShiftCoef=shiftcoef;
  ShiftTFS=shifttfs;
}
...
//==============================================================================
void JShifting::AddZone(bool fromxml,const tdouble3 &posref
  ,const tdouble3 &vx,const tdouble3 &vy,const tdouble3 &vz)
{
  const unsigned id=GetCount();
  JShiftingZone* zo=new JShiftingZone(id,posref,vx,vy,vz);
  if(zo->GetUsePosMax()){
...
//==============================================================================
void JShifting::VisuConfig(std::string txhead,std::string txfoot){
...
//==============================================================================
template<bool first,bool dbl> void JShifting::InitCpuPosMax(unsigned n,unsigned pini
...
//==============================================================================
template<bool first,bool dbl> void JShifting::InitCpuPlanes(unsigned n,unsigned pini
...
//==============================================================================
void JShifting::InitCpu(unsigned n,unsigned pini,const tdouble3* pos,tfloat4* shiftposfs)const{
  const unsigned nz=GetCount();
  if(!nz)memset(shiftposfs+pini,0,sizeof(tfloat4)*n);   //shiftposfs[]=0
    ...
    while(nz){
      const JShiftingZone* zo1=Zones[cz];
      const JShiftingZone* zo2=(nz>1? Zones[cz+1]: NULL);
...
//==============================================================================
void JShifting::RunCpu(unsigned n,unsigned pini,double dt,const tfloat4* velrhop
...
/// Selecciona particulas para shifting e inicializa shiftposfs[].
//==============================================================================
void JShifting::InitGpu(unsigned n,unsigned pini,const double2* posxy,const double* posz
...
/// Calcula Shifting final para posicion de particulas.
//==============================================================================
void JShifting::RunGpu(unsigned n,unsigned pini,double dt,const float4* velrhop
  ,float4* shiftposfs,cudaStream_t stm)const
{
  const double coefumagn=dt*ShiftCoef*H;
