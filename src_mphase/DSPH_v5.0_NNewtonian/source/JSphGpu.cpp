...
void JSphGpu::PreInteraction_Forces(){
  ...
  if(MultiPhase)ViscEtaDtg=ArraysGpu->ReserveFloat(); //<vs_non-Newtonian>
  ...
  if(TVisco!=VISCO_Artificial)SpsGradvelg=ArraysGpu->ReserveSymatrix3f();
  if(MultiPhase) {//<vs_non-Newtonian>
  ...
  VelMax=sqrt(velmax);
  cudaMemset(ViscDtg,0,sizeof(float)*Np);           //ViscDtg[]=0
  //cudaMemset(ViscEtaDtg, 0, sizeof(float)*Np);      //ViscEtaDtg[]=0  //<vs_non-Newtonian>
  ViscDtMax=0;
  ViscEtaDtMax=0; //<vs_non-Newtonian>
