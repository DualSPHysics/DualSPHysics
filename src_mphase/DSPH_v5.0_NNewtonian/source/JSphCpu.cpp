...
void JSphCpu::PreInteractionVars_Forces(unsigned np,unsigned npb){
  ...
  if(Visco_etac) memset(Visco_etac,0,sizeof(float)*np);  //SpsGradvelc[]=(0,0,0,0,0,0).
  ...
  else ComputePress_NN(np,npb);  //<vs_non-Newtonian> 
}
//==============================================================================
/// Prepare variables for interaction functions.
/// Prepara variables para interaccion.
...
void JSphCpu::PreInteraction_Forces(){
  ...
  if(TVisco!=VISCO_Artificial)SpsGradvelc=ArraysCpu->ReserveSymatrix3f();

  if(MultiPhase) {//<vs_non-Newtonian>
    D_tensorc=ArraysCpu->ReserveSymatrix3f();
...
template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity> void JSphCpu::Interaction_Forces_ct5(const stinterparmsc &t,StInterResultc &res)const{
  if(MultiPhase){  //<vs_non-Newtonian_ini>
    if(Shifting){ const bool shift=true;
      ...
      else if(TVelGrad==VELGRAD_SPH)Interaction_ForcesCpuT_NN_SPH<tker,ftmode,tvisco,tdensity,shift>(t,res);
    }else{        const bool shift=false;
           if(TVelGrad==VELGRAD_FDA)Interaction_ForcesCpuT_NN_FDA<tker,ftmode,tvisco,tdensity,shift>(t,res);
      else if(TVelGrad==VELGRAD_SPH)Interaction_ForcesCpuT_NN_SPH<tker,ftmode,tvisco,tdensity,shift>(t,res);
    }
  }
  else  //<vs_non-Newtonian_end>
...
template<TpKernel tker,TpFtMode ftmode> void JSphCpu::Interaction_Forces_ct3(const stinterparmsc &t,StInterResultc &res)const{
  ...
  else if(TVisco==VISCO_ConstEq   )Interaction_Forces_ct4<tker,ftmode,VISCO_ConstEq   >(t,res);
}
//==============================================================================
...
//#include "JSphCpu_Tensors.cpp"
#include "JSphCpu_NN_SPH.cpp"
#include "JSphCpu_NN_FDA.cpp"
