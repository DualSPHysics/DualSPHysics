...
void JSph::LoadConfigParameters(const JXml *xml){
  ...
  switch(eparms.GetValueInt("RheologyTreatment",true,1)){
    ...
    default: Run_Exceptioon("Rheology treatment is not valid.");
  }

	if(MultiPhase)lamda= eparms.GetValueFloat("RelaxationDt", true, 0.2f);
  //-VelocityGradientType switch
  switch(eparms.GetValueInt("VelocityGradientType",true,1)) {
...
void JSph::LoadCaseConfig(const JSphCfgRun *cfg){
  ...
  //-Configuration for NN-MultiPhase.  //<vs_non-Newtonian_ini>
  if(MultiPhase) {
...
std::string JSph::GetViscoName(TpVisco tvisco){
  ...
  else if(tvisco==VISCO_ConstEq)tx="Constitutive equations";
  else tx="???";
  return(tx);
}
...
//==============================================================================
void JSph::InitMultiPhase(const JXml *sxml,std::string xmlpath) {
  Log->Print("");
  Log->Print("[Multi-phase configuration]\n");
  TiXmlNode* node=sxml->GetNodeSimple("case.execution.special.nnphases");
  ...
  PhaseCte=new StPhaseCte[PhaseCount]; 	memset(PhaseCte,0,sizeof(StPhaseCte)*PhaseCount);
  ...
  for(unsigned c=0; ele; c++) {
    sxml->CheckElementNames(ele,true,"rhop csound gamma visco tau_yield tau_max Bi_multi HBP_n HBP_m cohesion phi phasetype");
    ...
    // PhaseCte holds const. eq. multiphase constants
    PhaseCte[c].idbegin=bk->Begin;
    PhaseCte[c].count=bk->Count;
    ...
    PhaseCte[c].n_NN=sxml->ReadElementFloat(ele,"HBP_n","value");
    PhaseCte[c].m_NN=sxml->ReadElementFloat(ele,"HBP_m","value");
    PhaseCte[c].coh=sxml->ReadElementFloat(ele,"cohesion","value",true);
    PhaseCte[c].phi=sxml->ReadElementFloat(ele,"phi","value",true);
    PhaseCte[c].phasetype=sxml->ReadElementUnsigned(ele,"phasetype","value");   //only 0 in v 5.0 supported
    //Set PhaseCte that need calculating in the loop
    /*const float ctphi=sqrt(9.f + 12.f * (tan(PhaseCte[c].phi)*tan(PhaseCte[c].phi)));
    PhaseCte[c].DP_alpha = tan(PhaseCte[c].phi) / ctphi;
    PhaseCte[c].DP_kappa = 3 * PhaseCte[c].coh  / ctphi;*/
    //fin
    ele=ele->NextSiblingElement("phase");
  }
  //-Sort phases according mkfluid.
  for(unsigned c=0; c<PhaseCount-1; c++)for(unsigned c2=c+1; c2<PhaseCount; c2++)if(PhaseCte[c].mkfluid>PhaseCte[c2].mkfluid){
    StPhaseCte a=PhaseCte[c];
    PhaseCte[c]=PhaseCte[c2];
    PhaseCte[c2]=a;
  ...
  for(unsigned c=0; c<=PhaseCount-1; c++) {
    ...
    const StPhaseCte &ct=PhaseCte[c];
    Log->Printf("  Viscosity: %f",ct.visco);
  ...
  Log->Printf("Dt relaxation parameter: %f",lamda);
  Log->Print("");
}
...
//==============================================================================
void JSph::ConfigConstantsMP(){
  //Check if Cs0 is present for ALL phases
  bool Cs0_present=true;
  for(unsigned c=0; c<PhaseCount; c++)if(!PhaseArray[c].Cs0)Cs0_present=false;
  if(Cs0_present){
    //Compute a new Cs0 for all system
    CSP.cs0=0;
    for(unsigned c=0; c<PhaseCount; c++) {
      CSP.cs0=max(CSP.cs0,double(PhaseArray[c].Cs0));
      //CteB has already been calculated in InitMultiphase
      PhaseArray[c].mass=float(PhaseArray[c].rho*(Simulate2D ? Dp*Dp : Dp*Dp*Dp));
    }
  }
  else {
    //Compute a new CteB for all phases
    for(unsigned c=0; c<PhaseCount; c++) {
      PhaseArray[c].CteB=float(CSP.cs0*CSP.cs0*PhaseArray[c].rho/PhaseArray[c].Gamma);
      PhaseArray[c].mass=float(PhaseArray[c].rho*(Simulate2D ? Dp*Dp : Dp*Dp*Dp));
    }
  }
  CoefDtMin*=1.0e-5f;
...
//==============================================================================
/// Returns the name of the velocity gradient formulation in text format.
//==============================================================================
std::string JSph::GetVelGradName(TpVelGrad tvelgrad) {
  string tx;
  if(tvelgrad==VELGRAD_FDA)tx="FDA approach";
  else if(tvelgrad==VELGRAD_SPH)tx="SPH approach";
  else tx="???";
  return(tx);
}
